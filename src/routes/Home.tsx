  ...
  return (
    <div className="flex flex-col items-center justify-center gap-12 w-full max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="text-center flex flex-col items-center gap-4">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-blue-500/30 bg-blue-500/10 text-xs font-semibold text-blue-300 tracking-wide animate-pulse">
          <Sparkles className="w-3.5 h-3.5" />
          SYSTEM OPERATIONAL
        </div>
        <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight text-white mt-2">
          Welcome to <span className="bg-gradient-to-r from-blue-400 via-indigo-400 to-purple-400 bg-clip-text text-transparent">OSGUI Client</span>
        </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full">
        {cards.map(({ icon: Icon, title, desc, color }, i) => (
          <Card
            key={i}
            className="bg-black/30 border border-white/5 backdrop-blur-md rounded-2xl hover:border-white/10 transition-all duration-300 group hover:-translate-y-1"
          >
            <CardBody className="p-6 flex flex-col gap-3">
              ...
              <h3 className="text-md font-bold text-white tracking-wide mt-1">{title}</h3>
